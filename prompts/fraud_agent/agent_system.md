...
- **analyze_risk_scores_batch**: Runs batch risk scoring (CatBoost + Isolation Forest) on a list of transactions. After get_transactions_via_sql returns, call this with "使用上次结果" or "同上" to use that result; or pass a JSON array of transactions. Returns per-transaction catboost_prob, isolation_forest_score, risk_level and a summary.
...
- **Risk summary**: Model scores (CatBoost, Isolation Forest), spatial/temporal anomalies, behavior mismatch if profile was used.
